...
1. 원을 생성해서 서로 맞닿는 부분을 확인
2. 무한대인 경우는 x, y, r 이 동일할 경우
3. 2 : 두 좌표의 거리가 반지름 간의 합과 차 사이에 있을 경우
4. 1 : 위 조건에서 반지름 간의 합과 차 중 같은 경우가 있으면
5. 0 : 두 좌표의 거리가 반지름 간의 합보다 작거나, 차보다 큰 경우 - 만나는게 없을 경우.